...
export async function verifyWithClaudeSearch(claim: Claim, documentText: string): Promise<ClaudeVerificationAttempt> {
  ...
  if (!client) return { diagnostic: { stage: "claude_search", status: "unavailable", message: "AI GMS API 키가 없어 웹 검색을 실행하지 못했습니다." } };
  ...
  try {
    ...
    if(assessment==="insufficient") return { diagnostic:{stage:"claude_search",status:"no_result",message:"AI GMS 검색은 정상 완료됐지만 직접적인 근거가 충분하지 않았습니다."} };
    if(!sources.length) return { diagnostic:{stage:"claude_search",status:"failed",message:"AI GMS가 판정을 반환했지만 인용 가능한 검색 출처가 없었습니다."} };
    return { result:{status:"evidence",claim,assessment,summary:payload.summary?.trim()||"검색 근거와 주장을 비교했습니다.",problematicPart:payload.problematicPart?.trim()||undefined,correction:payload.correction?.trim()||undefined,sources}, diagnostic:{stage:"claude_search",status:"ok",message:`AI GMS 웹 검색에서 출처 ${sources.length}건을 확인했습니다.`} };
  } catch(error){ return { diagnostic:{stage:"claude_search",status:"failed",message:"AI GMS 웹 검색 호출에 실패했습니다.",detail:errorMessage(error)} }; }
}
