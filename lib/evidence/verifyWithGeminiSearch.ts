...
export async function verifyWithGeminiSearch(claim: Claim, documentText: string): Promise<GeminiVerificationAttempt> {
  ...
  if (!apiKey) return { diagnostic: { stage: "gemini_search", status: "unavailable", message: "AI GMS API 키가 없어 Google 검색 기반 검증을 실행하지 못했습니다." } };
  try {
...
형식: {"assessment":"supported|conflict|insufficient","summary":"근거 비교 결과","problematicPart":"문제 표현 또는 빈 문자열","correction":"교정 문장 또는 빈 문자열"}` }] }], tools: [{ google_search: {} }], generationConfig: { temperature: 0.1, maxOutputTokens: 768 } }),
    ...
    if (assessment === "insufficient") return { diagnostic: { stage: "gemini_search", status: "no_result", message: "AI GMS Google 검색은 완료됐지만 직접적인 근거가 충분하지 않습니다." } };
    if (!sources.length) return { diagnostic: { stage: "gemini_search", status: "failed", message: "AI GMS가 판정은 반환했지만 검색 출처를 제공하지 않았습니다." } };
    return { result: { status: "evidence", claim, assessment, summary: payload.summary?.trim() || "Google 검색 근거와 주장을 비교했습니다.", problematicPart: payload.problematicPart?.trim() || undefined, correction: payload.correction?.trim() || undefined, sources }, diagnostic: { stage: "gemini_search", status: "ok", message: `AI GMS Google 검색에서 출처 ${sources.length}건을 확인했습니다.` } };
  } catch (error) { return { diagnostic: { stage: "gemini_search", status: "failed", message: "AI GMS Google 검색 호출에 실패했습니다.", detail: error instanceof Error ? error.message : String(error) } }; }
}
